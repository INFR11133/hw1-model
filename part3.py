...
def extend_state(s, f):
  for j in xrange(s.i+1,len(f)+1):
    if f[s.i:j] in tm: # cases 1&4: translate next phrase, irrespective of gap
      for phrase in tm[f[s.i:j]]:
        (lm_state, logprob) = add_log_probs(s.lm_state, phrase, j==len(f) and s.gap is None)
        yield (state(j, s.gap, lm_state), logprob, phrase)
      if s.gap is None:
  ...
  if s.gap is not None: # case 3: fill in gap
